...
def parse_api_v1_url(url: str) -> dict:
    ...
    if data:
        return {'access': 'api_v1', 'category': 'resources', **data.groupdict()}
    ...
    if data:
        return {'access': 'api_v1', 'category': 'datasets', **data.groupdict()}

    # Reuse get
    api_v1_reuse_format = re.compile(r"^\/api\/1\/reuses\/(?P<reuse_id_or_slug>(\w|\-)+)\/?$")
    data = re.search(api_v1_reuse_format, url)
    if data:
        return {'access': 'api_v1', 'category': 'reuses', **data.groupdict()}
    ...
    if data:
        return {'access': 'api_v1', 'category': 'organizations', **data.groupdict()}
    return {}
